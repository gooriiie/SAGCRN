...
def generate_train_val_test(args):
    embedding_data = np.load('./PEMSD4/data.npz')
    original_df = pd.read_hdf('./PEMSD4/pems04.h5')
    # embedding_data = np.load('./PEMSD8/data.npz')
    # original_df = pd.read_hdf('./PEMSD8/pemsd8.h5')
    # embedding_data = np.load('./METRLA/data.npz')
    # original_df = pd.read_hdf('./METRLA/metr-la.h5')
    # embedding_data = np.load('./PEMSBAY/data.npz')
    # original_df = pd.read_hdf('./PEMSBAY/pems-bay.h5')

    # original_df = pd.read_hdf('./SEATTLE/seattle.h5')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, choices=['METRLA', 'PEMSBAY', 'PEMSD8', 'PEMSD4', 'DUNSAN'], default='METRLA', help='which dataset to run')
    parser.add_argument("--output_dir", type=str, default="METRLA/", help="Output directory.")
    parser.add_argument("--traffic_df_filename", type=str, default="METRLA/metr-la-restored.h5", help="Raw traffic readings.")
    args = parser.parse_args()
    args.output_dir = f'{args.dataset}/'
    if args.dataset == 'METRLA':
        args.traffic_df_filename = f'{args.dataset}/metr-la-restored.h5'
    elif args.dataset == 'PEMSBAY':
        args.traffic_df_filename = f'{args.dataset}/pems-bay-restored.h5'
    elif args.dataset == 'PEMSD8':
        args.traffic_df_filename = f'{args.dataset}/pemsd8-restored.h5'
    elif args.dataset == 'PEMSD4':
        args.traffic_df_filename = f'{args.dataset}/pems04-restored.h5'
    elif args.dataset == 'DUNSAN':
        args.traffic_df_filename = f'{args.dataset}/dunsan.h5'
    main(args)
