...
def zero_replacement_with_vae(dataset):
    ...
    data = pd.read_hdf(f'./{dataset}/{dataset_name}.h5')
    ...
    min_filled_data = data.apply(lambda col: col.fillna(col.min()), axis=1).values
    # min_filled_data = data.apply(lambda col: col.fillna(col.mean()), axis=0).values
    # min_filled_data = data.fillna(data.interpolate())
    min_filled_data = pd.DataFrame(min_filled_data)
    min_filled_data = min_filled_data.fillna(method='ffill')
    min_filled_data = min_filled_data.fillna(method='bfill')
    ...
    restored_data = np.round(restored_data, 1)
