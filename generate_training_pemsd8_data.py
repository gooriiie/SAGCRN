...
def generate_graph_seq2seq_io_data(
    ...
    df_value = np.expand_dims(df_value, axis=-1)   # (34272,1,207)
    missing_mask_value = np.expand_dims(missing_mask_value, axis=-1)
    data_list = [df_value]  # (1,34272,1,207)
    if add_time_in_day:
        # time_ind = (df.index.values - df.index.values.astype("datetime64[D]")) / np.timedelta64(1, "D")     # (34272,)

        from_date = datetime.strptime('20160701000000', '%Y%m%d%H%M%S')
        to_date = datetime.strptime('20160831235959', '%Y%m%d%H%M%S')
        ...
        date_array = np.array(date_df['date'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, choices=['PEMSD8'], default='PEMSD8', help='which dataset to run')
    parser.add_argument("--output_dir", type=str, default="PEMSD8/", help="Output directory.")
    parser.add_argument("--traffic_df_filename", type=str, default="PEMSD8/pemsd8.h5", help="Raw traffic readings.")
